 ...
 */
public class Driving extends Task {
	// --- Task Code ---//
	...
	public void update(double time) {
		if (time <= endTime) {
			simulation.getEnvironment().setTime(time);
			simulation.update();
			updateVisuals();
			simulator.repaint();
		} else
	...
	public double bind(Iterator<String> it) {
		try {
			...
			if (cmd.equals("image->angle")) {
				...
				return simulation.getEnvironment().getTime();
	...
	void doAccelerate(double fthw, double dthw, double dt) {
		...
		if (simcar.getSpeed() >= 10.0) {
			...
			accelBrake = .65 * (simulation.getEnvironment().getTime() / 3.0);
	...
	public static Image getImage(final String name) {
		URL url = Driving.class.getResource("images/" + name);
